    ...
    if (isReleaseJob()) {
        ...
        stage('Push docker') {
            ...
            for (int i = 0; i < versions.size(); i++) { //TODO switch to each after JENKINS-26481
                ...
                sh "docker tag hydrosphere/serving-runtime-tensorflow:${ver}-latest hydrosphere/serving-runtime-tensorflow:${ver}-${imageVersion}"
                sh "docker push hydrosphere/serving-runtime-tensorflow:${ver}-${imageVersion}"
            }
        }
        ...
        stage('Push docker') {
            if (env.BRANCH_NAME == "master") {
                for (int i = 0; i < versions.size(); i++) { //TODO switch to each after JENKINS-26481
                    def ver = versions.get(i)
                    sh "docker push hydrosphere/serving-runtime-tensorflow:${ver}-latest"
                }
            }
        }
    }
